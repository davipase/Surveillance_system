...
class Mocap_publisher(Node):

    def __init__(self):
        super().__init__('Mocap_publisher')
        self.publisher_ = self.create_publisher(VehicleVisualOdometry, '/qualisys_mocap_data', 10)
...
async def main(network_config_file_name):
    ...
    rclpy.init()
    mocap_publisher = Mocap_publisher()
    ...
    def on_packet(packet):
        ...
        if wanted_body is not None and wanted_body in body_index:
            ...
            for position, rotation in bodies:
                ...
                msg = VehicleVisualOdometry()
                msg.x = position[0]/1000
                msg.y = position[1]/1000
                msg.z = position[2]/1000
                rotation_np = np.asarray(rotation.matrix, dtype=np.float64).reshape(3, 3)
                r = R.from_matrix(rotation_np)
                msg.q[0] = r.as_quat()[0]
                msg.q[1] = r.as_quat()[1]
                msg.q[2] = r.as_quat()[2]
                msg.q[3] = r.as_quat()[3]
                print("rot quaternion: ",r.as_quat())
                print("rot angles: ",r.as_euler("xyz"),"\n")
                # print(msg)
                mocap_publisher.publisher_.publish(msg)
